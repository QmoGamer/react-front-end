...
class Theme extends Component {
  ...
  componentDidMount () {
    ...
    const { theme_id } = this.props.params

    this.dispatchThemeArticle( tab, 1 )
    this.props.dispatch( selectTheme( theme_id, 0  ) )
  }
  ...
  dispatchThemeArticle ( tab, page ) {
    ...
    if ( tab == "new" ) {
      this.props.dispatch( apiNewThemeArticle( location.origin, member_cookie, auth_token, theme_id, page ) )
        .then( res => this.handleApiResult( res ) )
    }
    else if ( tab == "hot" ) {
      this.props.dispatch( apiHotThemeArticle( location.origin, member_cookie, auth_token, theme_id, page ) )
        .then( res => this.handleApiResult( res ) )
    }
  }
  ...
  renderTitle() {
    ...
    theme.result.map((x, y)=>{ if( x.id == theme_id ) { theme_name = x.name }})
  ...
  renderSubThemeList () {
    ...
    const { theme } = this.props
    const { theme_id } = this.props.params
    const select_theme = theme.result.filter((x, y)=>x.id == theme_id)

    const render =  <div className={ styles.category_subs }>
                    {
  ...
  render () {
    ...
    let render_sub_theme_list = <Loader />
    let render_list = <Loader />
    let render_page = <Loader />

    if( loading == false ) {
      render_title = this.renderTitle()
      render_sub_theme_list = this.renderSubThemeList()
