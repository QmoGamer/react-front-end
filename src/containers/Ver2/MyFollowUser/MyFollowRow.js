...
class MyFollowUser extends Component {
  ...
  renderAction2 () {
    ...
    return <div onClick={ this.onCollect } className={ css_btn_collect }>收藏</div>
  }
  ...
  render () {
    ...
    console.log(data)
        ...
        <div className={ styles.info }>
          <div className={ styles.author }>{ data.nickname }</div>
          <div className={ styles.title }>{ data.title }</div>
          <div className={ styles.post_date }>{ data.post_date }</div>
        </div>
...
export default connect()(MyFollowUser)
